...
export default async function CustomerValue({ searchParams }: { searchParams: Promise<any> }) {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-gray-500 mb-4">Insight: Clientes frecuentes y gasto promedio.</p>
      ...
      <table className="w-full">
        <thead className="bg-gray-100 text-left">
          <tr><th>Cliente</th><th>Total Gastado</th><th>Gasto Prom.</th></tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
      ...
      <div className="mt-4 flex gap-2">
        {page > 1 && <a href={`?page=${page - 1}`} className="px-3 py-1 bg-gray-200 rounded">Atrás</a>}
        <a href={`?page=${page + 1}`} className="px-3 py-1 bg-gray-200 rounded">Siguiente</a>
      </div>
    </div>
  );
}