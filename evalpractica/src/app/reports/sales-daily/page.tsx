...
export default async function SalesDaily({ searchParams }: { searchParams: any }) {
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-gray-500 mb-4">Insight: Análisis de ingresos y tickets por día[cite: 6].</p>
      ...
      <table className="w-full border-collapse bg-white shadow-sm">
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className="border-t"><td>{r.fecha.toLocaleDateString()}</td><td>${r.total_ventas}</td><td>{r.tickets}</td></tr>
          ))}
        </tbody>
      </table>
    </div>
